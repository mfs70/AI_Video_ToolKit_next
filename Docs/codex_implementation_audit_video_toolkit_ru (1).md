...
# 14. PRODUCTION PLAYBACK PIPELINE AUDIT

## Критические проблемы найденные в BufferedVideoPlayer

### 1. Decoder recreation on seek

Сейчас seek выполняется через:

```text
Stop() -> FFmpeg restart -> decoder recreation
```
...
## Временное безопасное решение

### MainWindow.xaml.cs

Удалить из SeekToAsync:
...
- [ ] frame stepping больше не вызывает restart FFmpeg
- [ ] стрелки работают стабильно
- [ ] seek не уничтожает playback
- [ ] playback state сохраняется

---

# 15. BUFFEREDVIDEOPLAYER — КРИТИЧЕСКИЕ BOTTLENECK

## 15.1 BitmapSource.Create на каждый кадр
...
- [ ] нет Bitmap allocation на каждый frame
- [ ] playback smooth at 1080p60
- [ ] GC spikes минимальны
- [ ] UI latency стабильна

---

## 15.2 Buffer capacity слишком большой
...
```csharp
long targetBudget = 64L * 1024 * 1024;
```

и:

```csharp
return Math.Clamp(cap, 4, 24);
```
...
- [ ] no excessive pooling pressure

---

## 15.3 Audio pipeline

Сейчас звук обрабатывается через:
...
```csharp
var audioAllowed = enableAudio && speed <= 2.0;
```
...
# 16. TIMELINE CONTROL AUDIT

## Найдены проблемы

### Утечка handlers

Сейчас MouseLeftButtonDown подписывается внутри SetMarkers.
...
- [ ] timeline smooth after 30 min
- [ ] marker select stable

---

## FrameCursor visual tree issue
...
FrameCursor должен постоянно существовать в XAML.
...
# 17. MAINWINDOW — MVVM VIOLATIONS

Сейчас MainWindow.xaml.cs содержит:
...
- [ ] business logic removed from Window
- [ ] playback logic moved to services
- [ ] no direct playback control from view

---

# 18. PRODUCTION MONTAGE ARCHITECTURE
...
# 19. PRODUCTION PLAYBACK ARCHITECTURE
...
- [ ] no ffmpeg restart on seek
- [ ] frame stepping stable
- [ ] x16 playback stable
- [ ] playback resume stable

---

# 20. CODEX-READY PROMPTS

## Playback refactor

```text
Refactor BufferedVideoPlayer into a persistent decoder architecture.

Requirements:

1. Eliminate FFmpeg restart on seek.
2. Implement persistent decoder session.
3. Replace BitmapSource.Create with reusable WriteableBitmap.
4. Reduce allocations during playback.
5. Preserve frame-accurate playback.
6. Prevent audio drift.
7. Implement audio mute above x2 speed.
8. Use cancellation-safe async loops.
9. Prevent UI-thread blocking.
10. Maintain playback smoothness at 1080p60 and x16 speed.
```
...
```text
Refactor TimelineControl into a production-grade timeline.

Requirements:

1. Remove event subscription leaks.
2. Implement marker selection.
3. Implement marker dragging.
4. Prevent redraw storms.
5. Reuse visuals.
6. Keep FrameCursor persistent.
7. Support keyboard marker movement.
8. Maintain smooth playback interaction.
```

---

# 21. PERFORMANCE PROFILING PLAN
...
- Dispatcher stalls;
- synchronous Invoke usage;
- layout invalidation storms.
